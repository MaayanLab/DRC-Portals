...
for filename in "${files[@]}"; do
    ...
    if [ -f "/import/cypher/$filename" ]; then
        ...
        cypher-shell -p $NEO4J_PASSWORD -u $NEO4J_USERNAME -d $GRAPH_C2M2_DBNAME -f "/import/cypher/$filename"
    fi
done

# Make sure the C2M2 database is read-only once the data has been written
cypher-shell -p $NEO4J_PASSWORD -u $NEO4J_USERNAME "ALTER DATABASE $GRAPH_C2M2_DBNAME SET ACCESS READ ONLY"
