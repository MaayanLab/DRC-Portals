...
--- COLUMNS TO SHOW TO USER ---
    ...
    c2m2.biosample.sample_prep_method as sample_prep_method, c2m2.biosample.anatomy as anatomy, c2m2.biosample.biofluid as biofluid,
    ...
    ARRAY_AGG(DISTINCT c2m2.biosample.local_id) AS bios_array
...
--- JOIN ALL TABLES --- full outer join or inner join or left join or right join?
    ...
    left join c2m2.biofluid
        on (c2m2.biosample.biofluid = c2m2.biofluid.id)
    ...
    ---    on ((c2m2.protein_gene.protein = c2m2.protein.id) OR (c2m2.ptm.protein = c2m2.protein.id))
    ...
    --- Solution for future: require that project_local_id for subject/biosample pairs be the same. Also, 
    --- wherever possible, biosample should come from subject, i.e., there should be no biosample without 
    --- subject. Also, for a biosample/subject pair, if biosample is part of a collection, then the 
    --- collection for the subject should be the same (if at all), isn’t it?
    ...
    */
    ...
    /** Next several joins are commented in collection-centric table script **/
    ...
    */
    ...
    --- Mano: 2025/08/04: Add biosample_ptm, ptm, site_type, ptm_type, ptm_subtype
    left join c2m2.biosample_ptm 
        on (c2m2.biosample.local_id = c2m2.biosample_ptm.biosample_local_id and 
        c2m2.biosample.id_namespace = c2m2.biosample_ptm.biosample_id_namespace)
    left join c2m2.ptm
        on (c2m2.biosample_ptm.ptm = c2m2.ptm.id)
    ...
    left join c2m2.protein /* If there was a c2m2.biosample_protein table, that could be used here */
        on ((c2m2.protein_gene.protein = c2m2.protein.id) OR (c2m2.ptm.protein = c2m2.protein.id))
    ...
    c2m2.dcc.dcc_name, c2m2.dcc.dcc_abbreviation, c2m2.anatomy.name, c2m2.biofluid.name, c2m2.gene.name, c2m2.protein.id, 
    ...
    c2m2.subject_race_CV.name, c2m2.subject_granularity.name, c2m2.subject_sex.name, 
...
select project_id_namespace,count(*) from c2m2.ffl_biosample_cmp group by project_id_namespace order by project_id_namespace;
select dcc_name,count(*) from c2m2.ffl_biosample_cmp group by dcc_name order by dcc_name;
...
--- Cross-check after ffl_biosample_collection is ready
select count(*) from c2m2.collection_ptm;
select dcc_name,count(*) from c2m2.ffl_biosample_collection_cmp where ptm_site_type_name != '' group by dcc_name order by dcc_name;
select dcc_name,count(*) from c2m2.ffl_biosample_cmp where ptm_site_type_name != '' group by dcc_name order by dcc_name;
select dcc_name,count(*) from c2m2.ffl_collection_cmp where ptm_site_type_name != '' group by dcc_name order by dcc_name;
*/
