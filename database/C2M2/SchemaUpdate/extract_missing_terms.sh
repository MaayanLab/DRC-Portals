#!/bin/bash
# Call syntax: ./extract_missing_terms.sh <logfilename> <dbtablename>
...
psql -h localhost -p 5434 -U drc -d drc -c "\copy (SELECT * FROM c2m2.${dbtable} WHERE id IN (${ids_string})) TO '${outf}' WITH (FORMAT csv, DELIMITER E'\t', HEADER);"
