...
c2m2datapackage_helper = TableHelper('c2m2datapackage', ('id', 'dcc_asset_link',), pk_columns=('id',))
c2m2file_helper = TableHelper('c2m2file', ('id', 'datapackage_id', 'creation_time', 'persistent_id', 'size_in_bytes', 'file_format', 'data_type', 'assay_type',), pk_columns=('id',))
xidentity_helper = TableHelper('xidentity', ('id', 'type', 'label', 'description', 'c2m2file_id',), pk_columns=('id',))

with c2m2file_helper.writer() as c2m2file:
  with xidentity_helper.writer() as xidentity:
    with c2m2datapackage_helper.writer() as c2m2datapackage:
      for _, c2m2 in c2m2s.iterrows():
        ...
        c2m2datapackage_id = str(uuid5(uuid0, c2m2['link']))
        c2m2datapackage.writerow(dict(
          id=c2m2datapackage_id,
        ...
        with open(c2m2_extracted_files_tsv_path, 'r') as fr:
          ...
          for file in c2m2_files_reader:
            c2m2file_id = str(uuid5(uuid0, '\t'.join((c2m2datapackage_id, file['id_namespace'], file['local_id']))))
            c2m2file.writerow(dict(
              id=c2m2file_id,
              datapackage_id=c2m2datapackage_id,
