...
export const ClientCheckbox = ({query_key, value, label}: {query_key: 'type' | 'tags' | 'status' | 'cfde_specific', value: string, label: string}) => {
	...
	const query_parser = parseAsJson<OutreachParams>().withDefault({type: ['outreach', 'training'], tags:[], expand_filter: false, status: ['active', 'recurring', 'past'], cfde_specific: true})
