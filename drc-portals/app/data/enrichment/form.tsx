...
import { router_push, usePrevious } from './utils';
...
import EnrichrTermSearch from './enrichr_term_search';
import LibraryPicker from './library_picker';
import { NetworkSchema, EnrichmentParams } from './types';
...
const GeneSetForm = ({
    ...
    const addList = async () => {
        try {
            ...
            const query: {[key: string]: any} = {...combined_query}
    ...
    const verifyList = async (input: Array<string>) => {
        try {
            ...
            const verified:Array<string> = await (
                await fetch(`${process.env.NEXT_PUBLIC_GSE}/api/enrichment/terms_and_genes`, {
                    method: 'POST',
                    body: JSON.stringify({
                        input
                    }),
        ...
        return (
            <Button variant='outlined' color="secondary"
                onClick={()=>{
					...
					if (collapse === undefined) router_push(router, pathname, {...query, collapse: 'true'})
                    else router_push(router, pathname, query)
                }}
            >
                Expand Form
            </Button>
                    ...
                    <Grid container alignItems={"center"} spacing={1}>
                        ...
                        { fullWidth && 
                            <Grid item xs={fullWidth? 6: 12} sx={{textAlign: "right"}}>
                                <Button 
                                    onClick={()=>{
                                        const gene_set = example?.gene_set || ''
                                    ...
                                    <Tooltip title={`Filter out genes that are not in multiple libraries.`}>
                                        <Slider 
                                            ...
                                            value={min_lib || 1}
                                                ...
                                                setQuery({
                                                    ...query,
                                                    min_lib: typeof nv === 'number' ? nv: nv[0]
                                                })
                                                // router_push(router, pathname, {
                                                //     ...rest,
                                                //     min_lib: nv
                                    ...
                                    <Tooltip title={`Filter out genes with fewer connections`}>
                                        <Slider 
                                            ...
                                            onChange={(e, nv)=>{
                                                ...
                                                setQuery({
                                                    ...query,
                                                    gene_degree: typeof nv === 'number' ? nv: nv[0]
                                                })
                                            }}
                                            sx={{width: "100%"}}
                                            min={1}
                                            max={libraries.reduce((acc, i)=>(acc+(i.limit || 5)), 0) || 5}
                                            marks
                                            valueLabelDisplay='auto'
                                    ...
                                    <Tooltip title={`Filter out terms with fewer connections`}>
                                        <Slider 
                                            ...
                                            onChange={(e, nv)=>{
                                                ...
                                                setQuery({
                                                    ...query,
                                                    term_degree: typeof nv === 'number' ? nv: nv[0]
                                                })
                                            }}
                                            sx={{width: "100%"}}
                                            min={1}
                                    ...
                                    <Tooltip title={`How many genes should the knowledge graph return? (Prioritized by gene connectivity)`}>
                                        <Slider 
                                            ...
                                            onChange={(e, nv)=>{
                                                ...
                                                setQuery({
                                                    ...query,
                                                    gene_limit: typeof nv === 'number' ? nv: nv[0]
                                                })
                                            }}
                                            sx={{width: "100%"}}
                                            min={1}
