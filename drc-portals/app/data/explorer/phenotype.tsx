...
import { mdiHumanMaleHeightVariant } from '@mdi/js';
...
const Phenotype = ({ data, isConnectable }: {data: {update_input: Function}, isConnectable: boolean}) => {
  ...
  const { data: options } =  trpc.autocomplete.useQuery({
        ...
        facet: ["type:\"disease or phenotype\"", "type:\"disease\"", "type:\"phenotype\""],
    ...
    <CardContent>
        ...
        <Grid item xs={12}>
          <Stack direction={"row"} spacing={1} alignItems={"center"}>
            <Avatar sx={{backgroundColor: orange[100], color: 'black'}}><Icon path={mdiHumanMaleHeightVariant} size={2}/></Avatar>
            <Typography variant='h3'>Phenotypes and Diseases</Typography>
          </Stack>
        </Grid>
        <Grid item xs={12}>
          <Autocomplete
              ...
              renderInput={(params) => <TextField placeholder='Enter phenotype name' {...params} label="Enter Phenotype or Disease Name" />}
          />
        </Grid>
	    </Grid>
    </CardContent>
