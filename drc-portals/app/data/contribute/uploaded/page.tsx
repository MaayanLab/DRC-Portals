...
export default async function UserFiles() {
    ...
    if (!session.user.email) throw new Error('Missing email or user not configured')
    ...
    if (user.role === 'UPLOADER'){
        ...
        return (
            ...
            if (!user.dcc) throw new Error('user not assigned dcc')
            const userFiles = await prisma.dccAsset.findMany({
                include: {
                    dcc: {
                        select: {
            ...
            const symbolUserFiles = userFiles.map((userFile) => {
                let approvedSymbol = <Button> Approve Upload</Button>
                if (userFile.approved) {
                    approvedSymbol = <BsCheckCircle />
                }
                return ({ ...userFile, approvedSymbol })
        ...
        const symbolUserFiles = userFiles.map((userFile) => {
            let approvedSymbol = <ApprovalBtn {...userFile}/>
            if (userFile.approved) {
                approvedSymbol = <BsCheckCircleFill />
            }
            return ({ ...userFile, approvedSymbol })
        })
