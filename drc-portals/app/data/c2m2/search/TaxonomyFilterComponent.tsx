...
export default async function TaxonomyFilterComponent({ q, filterClause, maxCount, main_table }: { q: string, filterClause: SQL, maxCount: number, main_table: string }) {
  try {
    const query = SQL.template`
      WITH taxres_full AS (
        SELECT COALESCE(ncbi_taxonomy_name, 'Unspecified') AS taxonomy_name 
