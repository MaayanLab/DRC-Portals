...
export default async function FilesBiosampleTableComponent({ searchParams, filterClause, fileBiosTblOffset, limit, file_count_limit_bios }: { searchParams: any, filterClause: SQL, fileBiosTblOffset: number, limit: number, file_count_limit_bios: number }): Promise<JSX.Element> {
    ...
    try {
        ...
        const filesBios_table_columnsToIgnore: string[] = ['id_namespace', 'project_id_namespace', 'file_id_namespace', 'biosample_id_namespace', 'md5', 'sha256', 'file_format', 'compression_format', 'assay_type', 'analysis_type', 'data_type', 'searchable']; // added md5 and sha256 to ignore columns
        const {
